...
name: design-slides
description: |
  クライアントプレゼンテーション用の高品質でデザイン重視のHTMLスライドデッキを生成するスキル。
  単一の自己完結型HTMLファイルとして出力し、CSSデザインシステム・テーマ・インタラクティブナビゲーションを含む。
  ユーザーが「スライド作成」「プレゼン資料作成」「提案書作成」「ピッチデッキ作成」「クライアント向け資料」
  「slide」「presentation」「proposal deck」といったキーワードを使った場合に必ずこのスキルを使用する。
  スライドやプレゼン資料の新規作成が求められた場合は、明示的にスラッシュコマンドで呼ばれなくてもこのスキルを適用する。
---

# プレミアム クライアント向けスライド生成
